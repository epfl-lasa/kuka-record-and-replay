...
RobotInterface::Status record_and_replay_module::RobotUpdateCore(){
    ...
    case PREPARE:
            ...
            // finally we check if we are close to the starting configuration, in which case we can switch to replaying the trajectory
            if((joint_sensors_.GetJointPositions() - trajectory_[0]).Norm()<0.001)
                SwitchState(REPLAY);
            break;
    }
    {
